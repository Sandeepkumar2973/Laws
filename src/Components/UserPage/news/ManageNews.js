...
const ManageAllNews = () => {
  ...
  const fetchAllArticles = async () => {
    try {
      const { data } = await axios.get(`${mod.api_url}/api/v1/news/news_get`);
  ...
  const handleStatusToggle = async (slug, currentStatus) => {
    ...
    try {
      await axios.put(
        `${mod.api_url}/api/v1/news/news_status/${slug}`,
        { status: newStatus },
        config
      );

      setArticles((prevStory) =>
        prevStory.map((article) =>
  ...
  const handleDelete = async (slug) => {
    ...
    try {
      await axios.delete(
        `${mod.api_url}/api/v1/news/news_delete/${slug}`,
        config
      );
                    ...
                    <Tr key={article?._id}>
                      ...
                      <Td border="1px solid #ccc">
                        <Button
                          ...
                          onClick={() =>
                            handleStatusToggle(article?.slug, article?.status)
                          }
                        >
